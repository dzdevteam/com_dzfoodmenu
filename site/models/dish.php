 ...
 * Dzfoodmenu model.
 */
class DzfoodmenuModelDish extends JModelForm
{
	...
	protected function populateState()
	{
        ...
        if (JFactory::getApplication()->input->get('layout') == 'edit') {
            $id = JFactory::getApplication()->getUserState('com_dzfoodmenu.edit.dish.id');
        } else {
            $id = JFactory::getApplication()->input->get('id');
            JFactory::getApplication()->setUserState('com_dzfoodmenu.edit.dish.id', $id);
        }
		$this->setState('dish.id', $id);

		// Load the parameters.
		$params = $app->getParams();
        $params_array = $params->toArray();
        if(isset($params_array['item_id'])){
            $this->setState('dish.id', $params_array['item_id']);
        }
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int)$this->getState('dish.id');
	...
	{
		$id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('dish.id');
        $state = (!empty($data['state'])) ? 1 : 0;
        $user = JFactory::getUser();

        if($id) {
            //Check the user can edit this item
            $authorised = $user->authorise('core.edit', 'com_dzfoodmenu.dish.'.$id) || $authorised = $user->authorise('core.edit.own', 'com_dzfoodmenu.dish.'.$id);
            if($user->authorise('core.edit.state', 'com_dzfoodmenu.dish.'.$id) !== true && $state == 1){ //The user cannot edit the state of the item.
                $data['state'] = 0;
            }
        } else {
    ...
    {
        $id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('dish.id');
        if(JFactory::getUser()->authorise('core.delete', 'com_dzfoodmenu.dish.'.$id) !== true){
            JError::raiseError(403, JText::_('JERROR_ALERTNOAUTHOR'));
            return false;
        }
        $table = $this->getTable();
